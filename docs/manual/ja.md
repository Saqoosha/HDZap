...
- **ファームウェア**：M5StickS3 の `git describe` ビルドバージョン。M5 と接続して GATT 情報を取得したあとだけ表示されます。アプリ側のメジャーバージョンと一致しない場合は **赤字** + 不一致警告 — wire format が合うように再フラッシュしてください。同じ行は **設定 → デバイス → M5StickS3** の **バージョン** にも出ます。
