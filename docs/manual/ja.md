...
2. シート上部のインライン **レース** セクションでレース時間（既定 90 秒）と目標ラップ数を調整。
3. 設定シートを閉じる。
...
設定シートは「状態を上に、詳細はサブ画面に」という iOS 標準の設定アプリと同じドリルダウン構成になっています。シート最上部にインラインの **レース** セクションがあり、その下に **デバイス（Device）** と **アプリ（App）** の各サブ画面へ進むナビ行が並びます。各行は右側に現在値が見えます（Apple 設定アプリと同じパターン）。
...
- **ラップ読み上げ**：ON / OFF
- **ベストラップを通知**：ベスト更新を読み上げで強調
- **言語**：日本語、英語など
- **ボイス**：システム標準 + インストール済みの追加ボイス
- **レート**：読み上げ速度
- **ピッチ**：声の高さ
- **テスト**：現在の設定で読み上げを試す
- **リセット**：ラップ読み上げの設定を初期化
