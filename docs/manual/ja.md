...
6. **動作確認**：設定シート → **デバイス** → **OSD レイアウト** → **テスト OSD を送信** をタップ。Goggle に現在時刻が表示されれば bind 成功です。
...
2. シート上部のインライン **フォーマット** セクションで **レース時間**（既定 90 秒）と **目標ラップ数** を調整（**目標ペース** は自動計算）。
3. 設定シートを閉じる。
...
設定シートは「状態を上に、詳細はサブ画面に」という iOS 標準の設定アプリと同じドリルダウン構成になっています。シート最上部にインラインの **フォーマット** セクションがあり、その下に **デバイス** と **アプリ** の各サブ画面へ進むナビ行が並びます。各行は右側に現在値が見えます（Apple 設定アプリと同じパターン）。
...
- **Bluetooth 名**（接続中だけ表示）：M5StickS3 の名前を変更するドリルダウン。UTF-8 で最大 20 バイト、保存後 1 度だけ自動再起動し、iPhone も自動で繋ぎ直します。詳細は [Chapter 5 → M5StickS3 の名前を変更する（任意）](#m5sticks3-の名前を変更する任意)。
...
- **表示する行**：**Time / Lap / Pace / Diff** を個別に表示／非表示。隠した行は詰めて表示されるのでブロックは常にコンパクトに保たれます
...
- **アプリのバージョン**：iOS アプリの `CFBundleShortVersionString`。常に表示。
- **ファームウェア**：M5StickS3 の `git describe` ビルドバージョン。M5 と接続して GATT 情報を取得したあとだけ表示されます。アプリ側のメジャーバージョンと一致しない場合は **赤字** + 不一致警告 — wire format が合うように再フラッシュしてください。同じ行は **設定 → Device → M5StickS3** の **バージョン** にも出ます。
