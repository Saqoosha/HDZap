...
- **残り秒数をカウントダウンする**：既定 OFF。ON にすると、セッション終了直前を選択中のボイスでカウントダウン（日本語は「じゅう、きゅう…」、英語は "ten, nine, ..."）。ラップ実況の途中に来た数字は、待たせずに読み飛ばします（時計とズレないため）。System 音声では **目標ペースとの差を読み上げる** を ON にすると実況が長くなる分、終盤で飛ぶ数字が少し増えます。Premium 音声は重ねて読むため影響しません
