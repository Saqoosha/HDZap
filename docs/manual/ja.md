...
6. [HDZero Goggle と bind する](#6-hdzero-goggle-と-bind-する)
7. [レースする](#7-レースする)
8. [レースのあとに](#8-レースのあとに)
9. [設定リファレンス](#9-設定リファレンス)
10. [うまくいかないとき（トラブルシューティング）](#10-うまくいかないときトラブルシューティング)
11. [付録](#11-付録)
...
HDZap は、**iPhone で計測したラップタイムを HDZero Goggle の OSD に表示する** ためのシステムです。**パイロットとは別に、計測してくれるスポッターが1人必要** な、二人組での運用を前提としています。

```mermaid
flowchart LR
    A["iPhone<br/>HDZap App<br/><i>スポッター</i>"] -- Bluetooth --> B["M5StickS3<br/><i>中継器</i>"]
    B -- ESP-NOW --> C["HDZero Goggle<br/><i>パイロット</i>"]
```
...
- **HDZero Goggle**：パイロットが装着する FPV ゴーグル。内蔵の ELRS Backpack を通して OSD（画面オーバーレイ）にラップタイムを表示します。

> 💡 **iPhone アプリだけでも使えます。** タイム計測、ラップ履歴、音声読み上げなどはアプリ単体で完結します。M5StickS3 と Goggle を追加すると、それに加えてラップタイムが Goggle の OSD にも表示されるようになります。

### 用語ミニマップ

このあと頻繁に出てくる用語です。詳しい説明は [付録](#11-付録) にあります。

| 用語 | 1行説明 |
|---|---|
| **Goggle** | パイロットが装着する FPV ゴーグル。HDZero Goggle / HDZero Goggle 2 |
...
> ⚠️ **HDZero Goggle の ELRS Backpack ファームウェアが v1.5.5 以上であることが必須です。**
>
...
iPhone と M5StickS3 を Bluetooth で接続します。

### 手順

1. M5StickS3 の電源を入れます（LCD に表示が出ている状態）。
2. iPhone の HDZap アプリを開き、画面右上の **歯車アイコン（⚙️）** をタップして設定シートを開きます。
3. 設定シートの **デバイス（Device）** セクションにある **M5StickS3** をタップして接続画面に入ります。
4. **「スキャン」** ボタンをタップ。近くの M5StickS3 が **その他のデバイス（Other devices）** に表示されます。
5. **「HDZeroOSD」**（または下の[M5StickS3 の名前を変更する（任意）](#m5sticks3-の名前を変更する任意)で変えた任意の名前）の横の **「接続」** をタップ。

   <img src="images/05-bluetooth-before.png" alt="M5StickS3 サブ画面 接続前" width="320" />

6. 接続成功すると：
   - **接続済み（Connected）** セクションに緑の丸とデバイス名 + Disconnect ボタンが現れる
   - その下にバッテリ % と充電アイコンが出る
   - M5StickS3 側の LCD にも接続状態が出る

   <img src="images/05-bluetooth-after.png" alt="M5StickS3 サブ画面 接続後" width="320" />

これで iPhone と M5StickS3 の通信路ができました。次は Goggle との bind です。

### M5StickS3 の名前を変更する（任意）

M5StickS3 を複数台持っている場合、デフォルトの `HDZeroOSD` のままだと識別しづらくなります。接続中なら次の手順で名前を変えられます：

1. 設定 → **デバイス** → **M5StickS3**。
2. **Bluetooth name** をタップ。
...
> 💡 新しい名前はフラッシュメモリに保存されるので、電源を切っても残ります。`HDZeroOSD` を入れて保存し直せばデフォルトに戻ります。
...
```mermaid
flowchart TD
    A[ELRS Backpack の firmware に<br/>bind phrase を設定済み？] -- Yes --> B[6.1 既知の<br/>bind phrase を使う]
...
### 6.1 既知の bind phrase を使う

**こういう人向け**：ELRS Configurator で **bind phrase 入りのファームウェアを ELRS Backpack に書き込んだことがある** 人。bind phrase を控えている（または覚えている）。

#### 手順

1. アプリの設定シート → **デバイス** → **ゴーグルペアリング（Goggle pairing）** へ。
2. セグメント切り替えで **「バインドフレーズ」** を選択。
3. テキストフィールドに、ELRS Backpack に書き込んだのと同じ bind phrase を入力。
4. **「UID を適用」** ボタンをタップ。
5. 状態バナーの遷移を待ちます：`切り替え中…` → `確認中…` → `成功` 🎉
6. **動作確認**：設定シート → **デバイス** → **OSD レイアウト（OSD layout）** → **「テスト OSD を送信」** をタップ。Goggle に現在時刻が表示されれば bind 成功です。
...
5. M5StickS3 が bind ブロードキャストを受信して UID を抜き出し、画面に表示されます。
6. **「適用」** をタップ → `切り替え中…` → `確認中…` → `成功`。
7. **動作確認**：設定 → **デバイス** → **OSD レイアウト** → 「テスト OSD を送信」 で Goggle に時刻が出れば成功。
...
3. セグメント切り替えで **「手動 UID」** を選択。
4. **1 つの入力フィールド** にカンマ区切りで 6 つの数字を入力します。
5. **「UID を適用」** をタップ → `切り替え中…` → `確認中…` → `成功`。
6. **動作確認**：設定 → **デバイス** → **OSD レイアウト** → 「テスト OSD を送信」 で Goggle に時刻が出れば成功。
...
1. **Goggle を bind モードに入れる**：
   1. メニューで **Backpack On** にする
   2. **ELRS → Bind** を選択
   3. Goggle が「bind 待ち」状態になります
2. アプリの設定シート → **デバイス** → **ゴーグルペアリング** へ。
3. セグメント切り替えで **「新規ペアリング」** を選択。
4. **「新しいゴーグルとペアリング」** ボタンをタップ。M5StickS3 が bind パケットをブロードキャスト。
5. Goggle が受理すると binding 完了。
6. アプリ側の状態バナーが `確認中…` → `成功` になれば OK。
7. **動作確認**：設定 → **デバイス** → **OSD レイアウト** → 「テスト OSD を送信」 で Goggle に時刻が出れば成功。
...
bind が成功したら、必ずテスト OSD で確認してください。

1. 設定 → **デバイス** → **OSD レイアウト**。
2. **「テスト OSD を送信」** ボタンをタップ。
3. Goggle 画面下部に **`TEST OSD` + 現在の日付 + 時刻** の 3 行が表示されます（毎回タップする度に時刻が更新されるので、パケットが届いているのが目で見て分かります）。
   ...
   一度出ると消えません — 消したいときは同じ画面の **「OSD をクリア」** をタップしてください。

文字が出れば、すべての通信路（iPhone → M5StickS3 → Goggle）が正常です。
...
また、ゴーグルペアリング画面の **「前のゴーグルに戻す」** ボタンを使えば、いつでも前の UID に戻せます。

### うまくいかないとき

- **テスト OSD を押しても Goggle に何も出ない**
...
2. シート上部のインライン **フォーマット（Format）** セクションでレース時間（既定 90 秒）と目標ラップ数を調整。
3. 設定シートを閉じる。
...
レース中、Goggle 画面下部に 4 行のオーバーレイが出ます。各行は 50 列の OSD グリッドの中で中央寄せされます。
...
設定シートは「状態を上に、詳細はサブ画面に」という iOS 標準の設定アプリと同じドリルダウン構成になっています。シート最上部にインラインの **フォーマット（Format）** セクションがあり、その下に **デバイス（Device）** と **アプリ（App）** の各サブ画面へ進むナビ行が並びます。各行は右側に現在値が見えます（Apple 設定アプリと同じパターン）。

### フォーマット（Format）（ルートにインライン表示）

- **レース時間（Race time）**：60〜180 秒（5 秒刻み）
- **目標ラップ（Target lap）**：例 5L
- **目標ペース（Target pace）**：レース時間と目標ラップから自動計算（読み取り専用）

### デバイス → M5StickS3（接続）

- 上部にステータスドット + デバイス名（ルートから降りてきた行と同じ）
- **接続済み（Connected）**：デバイス名・識別子の先頭部分・バッテリ %・充電アイコン・**Disconnect** ボタン
- **Bluetooth name**（接続中だけ表示）：M5StickS3 の名前を変更するドリルダウン。UTF-8 で最大 20 バイト、保存後 1 度だけ自動再起動し、iPhone も自動で繋ぎ直します。詳細は [Chapter 5 → M5StickS3 の名前を変更する（任意）](#m5sticks3-の名前を変更する任意)。
- **その他のデバイス（Other devices）**：周辺の M5StickS3。各行に **Connect** ボタン
- **スキャン**：再スキャン

### デバイス → ゴーグルペアリング（Goggle pairing）

モード切り替えで「バインドフレーズ／手動 UID／新規ペアリング」のフォームを切り替えます。**TX UID キャプチャ** も同じ画面の下にあります。詳しいワークフローは [Chapter 6](#6-hdzero-goggle-と-bind-する)。

- **現在の UID（Current UID）**：M5StickS3 にいま入っている UID
- **UID を適用 / 新しいゴーグルとペアリング**：選んだフローを実行。状態バナーが `切り替え中…` → `確認中…` → `成功` / 自動ロールバックと進みます
- **前のゴーグルに戻す**：適用に失敗した直後に手動で巻き戻すボタン

### デバイス → OSD レイアウト（OSD layout）

ゴーグルの OSD のライブエディタ。画面上部に 4 行プレビューがあり、変更はリアルタイムでゴーグルにプッシュされるので、レースを走らせなくても見た目を確認できます。

- **Top row** スライダー：18 行のゴーグル画面のどこに OSD ブロックを置くか（1 = 最上、デフォルト = 下端揃え）
- **Alignment（配置）**：左 / 中央 / 右。すべての可視行に適用
- **Show rows**：**Time / Lap / Pace / Diff** を個別に表示／非表示。隠した行は詰めて表示されるのでブロックは常にコンパクトに保たれます
- **テスト OSD を送信**：iPhone の現在日時をゴーグルに 1 回だけ送る。タップごとに時刻が更新されるのでパケットが届いているか目視で確認できます
- **OSD をクリア**：ゴーグルのオーバーレイバッファを消す
- **レイアウトをリセット（Reset layout）**：エディタを既定値（下端揃え・中央配置・全行表示）に戻す

> ℹ️ 縦位置の調整は OSD レイアウトキャラクタリスティックに対応したファームウェアが必要です。古いファームでも配置と表示／非表示は使えますが、縦位置はライブで反映できません（その場合エディタが警告を表示します）。

### アプリ → ラップ読み上げ（Lap announcer）

- **ラップ読み上げ（Announce lap times）**：ON / OFF
- **ベストラップ宣言（Say "best lap" on new best）**：新しいベストラップが出たとき、読み上げに「best lap」を付ける
- **言語（Language）**：日本語、英語など
- **ボイス（Voice）**：システム標準 + インストール済みの追加ボイス
- **レート（Rate）**：読み上げ速度
- **ピッチ（Pitch）**：声の高さ
- **テスト（Test voice）**：現在の設定で読み上げを試す
- **リセット（Reset）**：ラップ読み上げの設定を初期化
...
| 症状 | 対処 |
|---|---|
| テスト OSD で Goggle に何も出ない | **まず Goggle Backpack のファームウェアが v1.5.5 以上か確認**（最頻原因）。次に [Chapter 6 の判定フロー](#判定フローチャートあなたはどれ) を最初からやり直す |
| OSD の文字が崩れる / 一部しか出ない | Goggle ファームウェアバージョンを再確認。次に周囲の 2.4 GHz 電波干渉（Wi-Fi、ドローン本体など）を疑う |
| bind 直後にテスト OSD が失敗する | アプリの「前のゴーグルに戻す」で前の状態に戻し、別の bind 方法を試す |
...
- **Backpack**：Goggle やプロポに付ける ESP32 モジュール（HDZero Goggle では内蔵）。映像信号とは別の制御チャネル。
