...
1. アプリ作成者から受け取った **TestFlight 招待リンク** を iPhone で開きます。
...
3. 設定シート内の **Bluetooth** セクションを探します。
4. **「スキャン」** ボタンをタップ。近くにある M5StickS3 が一覧に出ます。
5. **「HDZeroOSD」** という名前のデバイスの横の **「接続」** をタップ。

   <img src="images/05-bluetooth-before.png" alt="Bluetooth セクション 接続前" width="320" />
...
6. 接続成功すると：
   ...
   <img src="images/05-bluetooth-after.png" alt="Bluetooth セクション 接続後" width="320" />
...
2. アプリの設定シート → **TX UID キャプチャ** セクションへ。
...
7. **動作確認**：デバッグ → 「テスト OSD を送信」 で Goggle に時刻が出れば成功。
...
3. Goggle 画面下部に **`TEST OSD` + 現在の日付 + 時刻** の 3 行が表示されます（毎回タップする度に時刻が更新されるので、パケットが届いているのが目で見て分かります）。
   ...
   一度出ると消えません — 消したいときは同じ **デバッグ** セクションの **「OSD をクリア」** をタップしてください。
...
また、設定シートの **「前のゴーグルに戻す」** ボタンを使えば、いつでも前の UID に戻せます。
...
2. **レース** セクションでレース時間（既定 90 秒）と目標ラップ数を調整。
3. 設定シートを閉じる。
...
- 接続中デバイス、切断、スキャン、バッテリ % 表示。詳細は [Chapter 5](#5-m5sticks3-と-iphone-をペアリングbluetooth)。

### ペアリング / TX UID キャプチャ

- Goggle との bind。詳細は [Chapter 6](#6-hdzero-goggle-と-bind-する)。

### デバッグ

- **テスト OSD を送信**：bind 動作確認用に現在時刻を1回だけ送る。
- **OSD をクリア**：Goggle の OSD オーバーレイを消す。
