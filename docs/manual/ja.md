...
3. **「接続して書き込む」** をクリックする前に **「完全初期化する」** チェックボックスを決めます（初期状態は OFF）：
   - **初回フラッシュ時はチェックを入れてください**。M5StickS3 のフラッシュメモリを完全に消去してから書き込みます
...
6. プログレスバーが終わるまで待ちます（30〜90 秒ほど）。このクリック 1 回で接続・ダウンロード・（必要なら）消去・書き込みまで進むので、「書き込む」という別ボタンを探す必要はありません。
