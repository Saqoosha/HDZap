...
- **最後の数秒をカウントダウン**：既定 OFF。ON にすると、セッション終了直前を選択中のボイスでカウントダウン（日本語は「じゅう、きゅう…」、英語は "ten, nine, ..."）。
- **開始秒数**：5〜15 秒。既定 10。**最後の数秒をカウントダウン** が ON のときだけ表示。
