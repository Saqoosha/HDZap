...
- `main` — known-good release; CI deploys to production (`/flash/`, `/ja/`, `/`)
...
- **Branch protection**: `main` is protected against direct push (PR-only merge). The release script exclusively promotes through a PR, so this is not an obstacle. Hotfixes still go via PR — branch from `main`, fix, PR straight back to `main`, then bring `develop` up to date.
